...
#>
  ...
  begin {
    $Param = @{
      Command = $MyInvocation.MyCommand.Name
      Endpoint = $PSCmdlet.ParameterSetName
      Format = @{ Body = @{ resources = @('assignment_rule','description','id','name') }}
    }
  }
...
#>
  [CmdletBinding(DefaultParameterSetName='/devices/queries/devices/v1:get',SupportsShouldProcess)]
  param(
    ...
    [string[]]$Id,
    [Parameter(ParameterSetName='/devices/queries/devices/v1:get',Position=1)]
    ...
    [string]$Filter,
    [Parameter(ParameterSetName='/devices/queries/devices/v1:get',Position=2)]
    ...
    [string]$Sort,
    [Parameter(ParameterSetName='/devices/queries/devices/v1:get',Position=3)]
    ...
    [int32]$Limit,
    [Parameter(ParameterSetName='/devices/queries/devices/v1:get',Position=4)]
    ...
    [Alias('fields')]
    [string[]]$Field,
    [Parameter(ParameterSetName='/devices/queries/devices/v1:get')]
  ...
  begin {
    $Param = @{
      Command = $MyInvocation.MyCommand.Name
      Endpoint = $PSCmdlet.ParameterSetName
      Format = if ($PSCmdlet.ParameterSetName -eq '/devices/entities/online-state/v1:get') {
        @{ Query = @('ids') }
      } else {
        @{ Body = @{ root = @('ids') }; Query = @('fields','filter','limit','sort','offset') }
      }
...
#>
  ...
  begin {
    $Param = @{
      ...
      Format = @{ Body = @{ resources = @('assignment_rule','description','group_type','name') }}
    }
