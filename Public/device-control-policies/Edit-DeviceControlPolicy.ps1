...
#>
    [CmdletBinding(DefaultParameterSetName = 'updateDeviceControlPolicies')]
    [OutputType()]
    param(
        [Parameter(
            ParameterSetName = 'Array',
            HelpMessage = 'An array containing multiple policies to update using a single request',
            Mandatory = $true)]
        [array] $Array,
    ...
    DynamicParam {
        ...
        $Endpoints = @('updateDeviceControlPolicies')
